...
def plot_confusion_matrix_with_counts_and_percentages(cm, ax):
    ...
    annot = np.array([["{}\n({:.2%})".format(cm[i, j], cm[i, j] / n) for j in range(cm.shape[1])] for i in range(cm.shape[0])])
    sns.heatmap(cm, annot=annot, fmt='', ax=ax, cmap='Greens', cbar=False, linewidths=1, linecolor='black')
    ax.set_xlabel('Tahmin Değerleri', fontsize=14)
    ax.set_ylabel('Gerçek Değerler', fontsize=14)
    ax.set_xticklabels(['Pozitif', 'Negatif'], fontsize=12)
    ax.set_yticklabels(['Pozitif', 'Negatif'], fontsize=12, rotation=0)
...
plt.plot(val_accuracy, label='Test Doğruluğu')
...
plt.title('Alıcı işletim karakteristiği (AİK)')
plt.legend(loc="lower right")
plt.show()
